...
namespace Bitonic
{
  ...
  // --------------------------------------------
  template<typename Obj> 
  std::pair<Obj*,int> _merge( bool up, Obj* objs, int len )
  {
  ...
  // --------------------------------------------
  template<typename Obj> 
  std::pair<Obj*,int> _sort( bool up, Obj* objs, int len, int cpt )
  {
    if (len <= 1) return std::make_pair(objs,1);
    std::pair<Obj*,int> first;
    std::pair<Obj*,int> second;
    int rang;

    if (cpt < 1)
    {
#     pragma omp parallel shared (first, second) private (rang)
      { 
        rang = omp_get_thread_num();
        if (rang % 2 == 0) first  = _sort(true , objs, len/2, cpt+1); 
        if (rang % 2 == 1) second = _sort(false, objs + len/2, len - (len/2), cpt+1);
      }
    }
    else
    {
      first  = _sort(true , objs, len/2, cpt+1);
      second = _sort(false, objs + len/2, len - (len/2), cpt+1);
    };
    return _merge(up, first.first, first.second + second.second );
  }
  // --------------------------------------------
  template<typename Obj> 
  std::vector<Obj>& sort( bool up, std::vector<Obj>& x )
  {
